...
lib_inc = "../../include"  # Ajuste se o seu header estiver em outro caminho


#############################################################
# functions
def page_reclaims(prog):
    # No Ubuntu/Linux usamos o time -v (verbose) no lugar do -l do macOS
    com = "./run_test.sh /usr/bin/time -v ./" + bin_folder + prog
    pipe = cmd.Popen(com.split(), stdout=cmd.PIPE, stderr=cmd.PIPE)
    output, errput = pipe.communicate()

    # A saída do GNU time no Linux para page reclaims é diferente
    m = re.search(
...
def cmp_output(prog, to_compare):
    ...
    # Corrigido: É necessário decodificar os bytes para string antes de comparar
...
if pr1 < 255:
    print(
        Fore.RED
        + "With fewer than 255 pages, the available memory is insufficient.: 0"
        + Style.RESET_ALL
    )
elif pr1 > 1023:
    print(
        Fore.RED
        + "With 1023 pages and more, the malloc works but consumes a minimum page to each allocation: 1"
        + Style.RESET_ALL
    )
elif pr1 > 513 and pr1 < 1022:
    print(
        Fore.RED
        + "With 513 to 1022 pages, the malloc works but the overhead is too important: 2"
        + Style.RESET_ALL
    )
elif pr1 > 313 and pr1 < 512:
    print(
        Fore.RED
        + "With 313 to 512 pages, the malloc works but the overhead is very important: 3"
        + Style.RESET_ALL
    )
elif pr1 > 273 and pr1 < 312:
    print(
        Fore.YELLOW
        + "With 273 to 312 pages, the malloc works but the overhead is important: 4"
        + Style.RESET_ALL
    )
elif pr1 >= 255 and pr1 <= 272:
    print(
        Fore.GREEN
        + "With 255 to 272 pages, the malloc works and the overhead is reasonable: 5"
        + Style.RESET_ALL
    )
